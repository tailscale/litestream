...
func (db *DB) streamWALSegment(ctx context.Context, hdr *StreamRecordHeader, r io.Reader) error {
	println("dbg/stream.wal", hdr.Pos().String())
	...
	db.Logger.Printf("wal segment applied")
	println("")
...
func invalidateSHMFile(dbPath string) error {
	...
	if _, err := db.Exec(`PRAGMA wal_checkpoint(PASSIVE)`); err != nil {
		println("dbg/PASSIVE.ERROR, exiting")
		os.Exit(1)
	...
	fmt.Printf("dbg/trunc %v\n", row)

	//var n int
	//if err := db.QueryRow(`SELECT COUNT(*) FROM _litestream_seq`).Scan(&n); err != nil {
	//	println("dbg/reopen.select", err.Error())
	//	// return fmt.Errorf("reopen select: %w", err)
	//}
